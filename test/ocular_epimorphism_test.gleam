...
pub fn epi_iso_composition_test() {
  ...
  // Int <-> Float iso
  let int_float_iso =
    ocular.iso(get: fn(n: Int) { int.to_float(n) }, reverse: fn(f: Float) {
      float.truncate(f)
    })
...
pub fn epi_epi_composition_test() {
  ...
  let int_positive_epi =
    Epimorphism(get: fn(n: Int) { Ok(n > 0) }, reverse: fn(b: Bool) {
      case b {
        True -> 1
        False -> 0
      }
...
pub fn lens_epi_composition_test() {
  ...
  let user_name_lens =
    ocular.lens(get: fn(u: User) { u.name }, set: fn(name, _u) {
      User(name: name)
    })
...
pub fn lens_from_iso_test() {
  // Int <-> String iso
  let int_string_iso =
    ocular.iso(get: int.to_string, reverse: fn(s) {
      case int.parse(s) {
        Ok(n) -> n
        Error(_) -> 0
      }
